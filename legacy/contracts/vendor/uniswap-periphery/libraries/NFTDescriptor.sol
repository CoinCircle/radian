...
import '../../uniswap/libraries/BitMath.sol';
...
library NFTDescriptor {
    ...
    function addressToString(address addr) internal pure returns (string memory) {
        return HexStrings.toHexString(uint160(addr), 20);
    }

    function generateSVGImage(ConstructTokenURIParams memory params) internal pure returns (string memory svg) {
        NFTSVG.SVGParams memory svgParams;
        svgParams.quoteToken = addressToString(params.quoteTokenAddress);
        svgParams.baseToken = addressToString(params.baseTokenAddress);
        svgParams.poolAddress = params.poolAddress;
        svgParams.quoteTokenSymbol = params.quoteTokenSymbol;
        svgParams.baseTokenSymbol = params.baseTokenSymbol;
        svgParams.feeTier = feeToPercentString(params.fee);
        svgParams.tickLower = params.tickLower;
        svgParams.tickUpper = params.tickUpper;
        svgParams.tickSpacing = params.tickSpacing;
        svgParams.overRange = overRange(params.tickLower, params.tickUpper, params.tickCurrent);
        svgParams.tokenId = params.tokenId;
        svgParams.color0 = tokenToColorHex(uint256(uint160(params.quoteTokenAddress)), 136);
        svgParams.color1 = tokenToColorHex(uint256(uint160(params.baseTokenAddress)), 136);
        svgParams.color2 = tokenToColorHex(uint256(uint160(params.quoteTokenAddress)), 0);
        svgParams.color3 = tokenToColorHex(uint256(uint160(params.baseTokenAddress)), 0);
        svgParams.x1 = scale(getCircleCoord(uint256(uint160(params.quoteTokenAddress)), 16, params.tokenId), 0, 255, 16, 274);
        svgParams.y1 = scale(getCircleCoord(uint256(uint160(params.baseTokenAddress)), 16, params.tokenId), 0, 255, 100, 484);
        svgParams.x2 = scale(getCircleCoord(uint256(uint160(params.quoteTokenAddress)), 32, params.tokenId), 0, 255, 16, 274);
        svgParams.y2 = scale(getCircleCoord(uint256(uint160(params.baseTokenAddress)), 32, params.tokenId), 0, 255, 100, 484);
        svgParams.x3 = scale(getCircleCoord(uint256(uint160(params.quoteTokenAddress)), 48, params.tokenId), 0, 255, 16, 274);
        svgParams.y3 = scale(getCircleCoord(uint256(uint160(params.baseTokenAddress)), 48, params.tokenId), 0, 255, 100, 484);
