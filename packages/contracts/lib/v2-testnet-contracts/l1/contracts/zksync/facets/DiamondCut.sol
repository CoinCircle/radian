...
contract DiamondCutFacet is Base, IDiamondCut {
  ...
  function proposeTransparentUpgrade(
    ...
    s.upgrades.proposedUpgradeHash = upgradeProposalHash(
      _diamondCut,
      expectedProposalId,
      proposalSalt
    );
  ...
  function cancelUpgradeProposal(
    bytes32 _proposedUpgradeHash
  ) external onlyGovernor upgradeProposed {
  ...
  function securityCouncilUpgradeApprove(
    bytes32 _upgradeProposalHash
  ) external onlySecurityCouncil upgradeProposed {
  ...
  function executeUpgrade(
    ...
    uint256 currentProposalId = s.upgrades.currentProposalId;
    bytes32 executingProposalHash = upgradeProposalHash(
      _diamondCut,
      currentProposalId,
      _proposalSalt
    );
