...
contract('ERC20VotesComp', function (accounts) {
  ...
  describe('set delegation', function () {
    describe('call', function () {
      it('delegation with balance', async function () {
        ...
        expect(await this.token.getPriorVotes(holder, receipt.blockNumber)).to.be.bignumber.equal(
          supply,
        );
      });
    ...
    describe('with signature', function () {
      ...
      it('accept signed delegation', async function () {
        ...
        expect(
          await this.token.getPriorVotes(delegatorAddress, receipt.blockNumber - 1),
        ).to.be.bignumber.equal('0');
        await time.advanceBlock();
        expect(
          await this.token.getPriorVotes(delegatorAddress, receipt.blockNumber),
  ...
  describe('change delegation', function () {
    ...
    it('call', async function () {
      ...
      expect(await this.token.getPriorVotes(holder, receipt.blockNumber - 1)).to.be.bignumber.equal(
        supply,
      );
      expect(
        await this.token.getPriorVotes(holderDelegatee, receipt.blockNumber - 1),
      ).to.be.bignumber.equal('0');
      await time.advanceBlock();
      expect(await this.token.getPriorVotes(holder, receipt.blockNumber)).to.be.bignumber.equal(
        '0',
      ...
      expect(
        await this.token.getPriorVotes(holderDelegatee, receipt.blockNumber),
      ).to.be.bignumber.equal(supply);
    });
  });
  ...
  describe('transfers', function () {
    ...
    afterEach(async function () {
      ...
      await time.advanceBlock();
      expect(await this.token.getPriorVotes(holder, blockNumber)).to.be.bignumber.equal(
        this.holderVotes,
      );
      expect(await this.token.getPriorVotes(recipient, blockNumber)).to.be.bignumber.equal(
        this.recipientVotes,
      );
    });
  });
  ...
  describe('Compound test suite', function () {
    ...
    describe('numCheckpoints', function () {
      it('returns the number of checkpoints for a delegate', async function () {
        ...
        await time.advanceBlock();
        expect(
          await this.token.getPriorVotes(other1, t1.receipt.blockNumber),
        ).to.be.bignumber.equal('100');
        expect(
          await this.token.getPriorVotes(other1, t2.receipt.blockNumber),
        ).to.be.bignumber.equal('90');
        expect(
          await this.token.getPriorVotes(other1, t3.receipt.blockNumber),
        ).to.be.bignumber.equal('80');
        expect(
          await this.token.getPriorVotes(other1, t4.receipt.blockNumber),
        ).to.be.bignumber.equal('100');
      });
    ...
    describe('getPriorVotes', function () {
      ...
      it('returns the latest block if >= last checkpoint block', async function () {
        ...
        expect(
          await this.token.getPriorVotes(other1, t1.receipt.blockNumber + 1),
        ).to.be.bignumber.equal('10000000000000000000000000');
      });
      ...
      it('generally returns the voting balance at the appropriate checkpoint', async function () {
        ...
        expect(
          await this.token.getPriorVotes(other1, t2.receipt.blockNumber + 1),
        ).to.be.bignumber.equal('9999999999999999999999990');
        expect(
          await this.token.getPriorVotes(other1, t3.receipt.blockNumber),
        ).to.be.bignumber.equal('9999999999999999999999980');
        expect(
          await this.token.getPriorVotes(other1, t3.receipt.blockNumber + 1),
        ).to.be.bignumber.equal('9999999999999999999999980');
        expect(
          await this.token.getPriorVotes(other1, t4.receipt.blockNumber),
        ).to.be.bignumber.equal('10000000000000000000000000');
        expect(
          await this.token.getPriorVotes(other1, t4.receipt.blockNumber + 1),
        ).to.be.bignumber.equal('10000000000000000000000000');
      });
    });
  });
