...
contract('Radian', function(accounts) {
    ...
    describe("liquidateRAD", () => {
        it("Should return the RAD balance of the liquidator", async () => {
            ...
            assert.equal(newOnwerBalance.valueOf(), redemptionAmount.valueOf(), "Not all tokens got transferred");
        });
        it("Should burn the liquidator's RAD ", async () => {
            ...
            assert.equal((STANDARD_TOKEN_COUNT_1B.sub(liquidationAmount)).valueOf(), totalSupply.valueOf(), "Not all tokens got transferred");
        });
        it("Should execute a number of transfers equal to that of the amount of token addresses passed in", async () => {
            ...
            await rad.approve(radian.address, MAX_NUM);
            await radian.liquidateRAD([token0.address, token1.address, token2.address], liquidationAmount.valueOf());
            ...
            assert.equal(newOnwerBalance0.valueOf(), redemptionAmount.valueOf(), "Not all tokens got transferred");
            assert.equal(newOnwerBalance1.valueOf(), redemptionAmount.valueOf(), "Not all tokens got transferred");
            assert.equal(newOnwerBalance2.valueOf(), redemptionAmount.valueOf(), "Not all tokens got transferred");
        });
        it("Should function the same way even after more tokens have been added to Radian", async () => {
            ...
            assert.equal(newOnwerBalanceOne.valueOf(), (redemptionAmount.mul(2)).valueOf(), "Not all tokens got transferred");
            assert.equal(newOnwerBalanceTwo.toPrecision(10), redemptionAmountTwo.valueOf(), "Not all tokens got transferred");
        });
        it("Should not release a token if it is not included in the token array", async () => {
            ...
            assert.equal(newOnwerBalanceOne.valueOf(), redemptionAmount.valueOf(), "Not all tokens got transferred");
        ...
        it("Should give a user the same amount of each type of token", async () => {
            ...
            await rad.approve(radian.address, MAX_NUM);
            await radian.liquidateRAD([token[0].address, token[1].address, token[2].address, token[3].address, token[4].address, token[5].address, token[6].address, token[7].address, token[8].address, token[9].address], liquidationAmount.valueOf());

            var newBalance = [];
            for (var j = 0; j < 10; j++) {
                newBalance[j] = await token[j].balanceOf(CC_SUPPLY);
                assert.equal(newBalance[j].valueOf(), redemptionAmount.valueOf(), "Not all tokens got transferred");
            }
        });
        it("Should finish the distribution with all of the tokens, adding a new token each time", async () => {
            ...
            // Define amounts
            const liquidationAmount = [new BigNumber(1e18), new BigNumber(2.5e26), new BigNumber(123456000e18), new BigNumber(626543999e18)];
            const redemptionAmount = [new BigNumber(1e18), new BigNumber(2.5e26), new BigNumber(2.5000000025e26), new BigNumber(123456000e18),  new BigNumber(123456000.123e18), new BigNumber(164608000.219e18), new BigNumber(626543999e18), new BigNumber(626543999.626e18), new BigNumber(835391999.780e18), new BigNumber(1e27)];
            let ownerBalance;
            ...
            assert.equal(ownerBalance.toPrecision(12), redemptionAmount[0].toPrecision(12), "Not all tokens got transferred");
            await tokenOne.transfer(THROWAWAY, ownerBalance.valueOf(), {from: USER_0});

            const tokenTwo = await Token.new(radian.address, STANDARD_TOKEN_COUNT_1B);
            await radian.liquidateRAD([tokenOne.address, tokenTwo.address], liquidationAmount[1].valueOf(), {from: USER_1});
            ownerBalance = await tokenOne.balanceOf(USER_1);
            assert.equal(ownerBalance.toPrecision(12), redemptionAmount[1].toPrecision(12), "Not all tokens got transferred");
            await tokenOne.transfer(THROWAWAY, ownerBalance.valueOf(), {from: USER_1});
            ownerBalance = await tokenTwo.balanceOf(USER_1);
            assert.equal(ownerBalance.toPrecision(12), redemptionAmount[2].toPrecision(12), "Not all tokens got transferred");
            ...
            await radian.liquidateRAD([tokenOne.address, tokenTwo.address, tokenThree.address], liquidationAmount[2].valueOf(), {from: USER_2});
            ownerBalance = await tokenOne.balanceOf(USER_2);
            assert.equal(ownerBalance.toPrecision(12), redemptionAmount[3].toPrecision(12), "Not all tokens got transferred");
            await tokenOne.transfer(THROWAWAY, ownerBalance.valueOf(), {from: USER_2});
            ownerBalance = await tokenTwo.balanceOf(USER_2);
            assert.equal(ownerBalance.toPrecision(12), redemptionAmount[4].toPrecision(12), "Not all tokens got transferred");
            await tokenTwo.transfer(THROWAWAY, ownerBalance.valueOf(), {from: USER_2});
            ownerBalance = await tokenThree.balanceOf(USER_2);
            assert.equal(ownerBalance.toPrecision(12), redemptionAmount[5].toPrecision(12), "Not all tokens got transferred");
            ...
            await radian.liquidateRAD([tokenOne.address, tokenTwo.address, tokenThree.address, tokenFour.address], liquidationAmount[3].valueOf(), {from: USER_3});
            ownerBalance = await tokenOne.balanceOf(USER_3);
            assert.equal(ownerBalance.toPrecision(12), redemptionAmount[6].toPrecision(12), "Not all tokens got transferred");
            await tokenOne.transfer(THROWAWAY, ownerBalance.valueOf(), {from: USER_3});
            ownerBalance = await tokenTwo.balanceOf(USER_3);
            assert.equal(ownerBalance.toPrecision(12), redemptionAmount[7].toPrecision(12), "Not all tokens got transferred");
            await tokenTwo.transfer(THROWAWAY, ownerBalance.valueOf(), {from: USER_3});
            ownerBalance = await tokenThree.balanceOf(USER_3);
            assert.equal(ownerBalance.toPrecision(12), redemptionAmount[8].toPrecision(12), "Not all tokens got transferred");
            await tokenThree.transfer(THROWAWAY, ownerBalance.valueOf(), {from: USER_3});
            ownerBalance = await tokenFour.balanceOf(USER_3);
            assert.equal(ownerBalance.toPrecision(12), redemptionAmount[9].toPrecision(12), "Not all tokens got transferred");
    ...
    describe("removeToken", () => {
        ...
        it("Should send tokens to the owner and return the remainder to the owner", async () => {
            ...
            assert.equal(oldContractBalance.valueOf(), STANDARD_TOKEN_COUNT_1B.valueOf(), "Radian contract did not get the correct tokens initially");
            ...
            assert.equal(newOnwerBalance.valueOf(), STANDARD_TOKEN_COUNT_1B.valueOf(), "Tokens did not transfer correctly");
        });
    });
    ...
    describe("Multiple Transactions", () => {
        it("Multiple Transactions", async () => {
            ...
            await radian.liquidateRAD([token.address], amountContributed.valueOf(), {from: COMMUNITY_SUPPLY});
