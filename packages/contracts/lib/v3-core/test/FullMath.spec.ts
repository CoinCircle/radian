...
describe('FullMath', () => {
  ...
  describe('#mulDiv', () => {
    ...
    it('accurate with phantom overflow', async () => {
      ...
      expect(
        await fullMath.mulDiv(Q128, BigNumber.from(35).mul(Q128), BigNumber.from(8).mul(Q128)),
      ).to.eq(result);
    });
    ...
    it('accurate with phantom overflow and repeating decimal', async () => {
      ...
      expect(
        await fullMath.mulDiv(Q128, BigNumber.from(1000).mul(Q128), BigNumber.from(3000).mul(Q128)),
      ).to.eq(result);
    });
  });
  ...
  describe('#mulDivRoundingUp', () => {
    ...
    it('reverts on overflow with all max inputs', async () => {
      await expect(fullMath.mulDivRoundingUp(MaxUint256, MaxUint256, MaxUint256.sub(1))).to.be
        .reverted;
    });
  ...
  function pseudoRandomBigNumber() {
    return BigNumber.from(
      new Decimal(MaxUint256.toString()).mul(Math.random().toString()).round().toString(),
    );
  }
