...
contract Governance is Ownable, StandardToken {
  ...
  function vote(uint256 choice, uint256 contribution) public {
  ...
  function weightCalculation(uint256 _contribution) internal view returns (uint256) {
  ...
  function returnRAD(address[] voters) public onlyOwner {
    require(block.timestamp > endTimestamp);
  ...
  function changeStartTimestamp(uint256 newStartTime) public onlyOwner {
  ...
  function changeEndTimestamp(uint256 newEndTime) public onlyOwner {
  ...
  function changeNumOptions(uint256 newNumOptions) public onlyOwner {
