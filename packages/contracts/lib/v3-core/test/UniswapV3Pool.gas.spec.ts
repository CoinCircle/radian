  ...
  for (const feeProtocol of [0, 6]) {
    describe(feeProtocol > 0 ? 'fee is on' : 'fee is off', () => {
      ...
      const gasTestFixture = async ([wallet]: Wallet[]) => {
        ...
        const { swapExact0For1, swapToHigherPrice, mint, swapToLowerPrice } =
      ...
      beforeEach('load the fixture', async () => {
        ({ swapExact0For1, pool, mint, swapToHigherPrice, swapToLowerPrice } = await loadFixture(
          gasTestFixture,
        ));
      });
          ...
          describe(description, () => {
            it('new position mint first in range', async () => {
              await snapshotGasCost(
                mint(wallet.address, tickLower, tickUpper, expandTo18Decimals(1)),
              );
            });
            ...
            it('second position in same range', async () => {
              await mint(wallet.address, tickLower, tickUpper, expandTo18Decimals(1));
              await snapshotGasCost(
                mint(other.address, tickLower, tickUpper, expandTo18Decimals(1)),
              );
            });
      ...
      describe('#collect', () => {
        ...
        it('close to worst case', async () => {
          ...
          await snapshotGasCost(
            pool.collect(wallet.address, tickLower, tickUpper, MaxUint128, MaxUint128),
          );
        });
      });
