  ...
  return (
    <Popover className="relative bg-black">
      <div className="mx-auto max-w-7xl px-6">
        <div className="flex items-center justify-between border-b-2 border-gray-900 py-6 md:justify-start md:space-x-10">
          ...
          <Popover.Group as="nav" className="hidden space-x-10 md:flex">
            {links.map((link) => (
              <a key={link.label} href="#" className="text-base font-medium text-gray-500 hover:text-gray-900">
                {link.label}
              </a>
            ))}
          </Popover.Group>
          ...
          <div className="divide-y-2 divide-gray-50 rounded-lg bg-black shadow-lg ring-1 ring-black ring-opacity-5">
            ...
            <div className="space-y-6 py-6 px-5">
              <div className="grid grid-cols-2 gap-y-4 gap-x-8">
                {links.map((link) => (
                  <a key={link.label} href="#" className="text-base font-medium text-gray-900 hover:text-gray-700">
                    {link.label}
                  </a>
                ))}
              </div>
