  ...
  context('recover with valid signature', function () {
    ...
    context('with v=27 signature', function () {
      ...
      it('works with correct v value', async function () {
        ...
        expect(
          await this.ecdsa.recover_r_vs(TEST_MESSAGE, ...split(to2098Format(signature))),
        ).to.equal(signer);
      });
      ...
      it('rejects incorrect v value', async function () {
        ...
        expect(await this.ecdsa.recover_v_r_s(TEST_MESSAGE, ...split(signature))).to.not.equal(
          signer,
        );
        expect(
          await this.ecdsa.recover_r_vs(TEST_MESSAGE, ...split(to2098Format(signature))),
