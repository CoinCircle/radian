...
describe('Path', () => {
  ...
  it('js encoding works as expected', async () => {
    ...
    expectedPath =
      '0x' + tokenAddresses.map(tokenAddress => tokenAddress.slice(2).toLowerCase()).join('000bb8');
  ...
  describe('#hasMultiplePools / #decodeFirstPool / #skipToken / #getFirstPool', () => {
    ...
    it('works on first pool', async () => {
      ...
      expect(await path.decodeFirstPool(await path.getFirstPool(encodedPath))).to.deep.eq(
        firstPool,
      );
    });
  ...
  it('gas cost', async () => {
    await snapshotGasCost(
      path.getGasCostOfDecodeFirstPool(
        encodePath([tokenAddresses[0], tokenAddresses[1]], [FeeAmount.MEDIUM]),
      ),
    );
  });
});
