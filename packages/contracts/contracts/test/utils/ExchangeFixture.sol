...
contract ExchangeFixture is Test {
  ...
  function exchangeCreatePool(address _tokenA, address _tokenB) public returns (ExchangePool) {
    ...
    address poolAddress = nftManager.createAndInitializePoolIfNecessary(token0, token1, FEE_MEDIUM, encodePriceSqrt(1, 1));
    ...
    PoolAddress.PoolKey memory poolKey =
        PoolAddress.PoolKey({token0: token0, token1: token1, fee: FEE_MEDIUM});
  ...
  function exchangeAddLiquidity(address _tokenA, address _tokenB) internal {
        ...
        address token0 = address(tokenA) < address(tokenB) ? address(tokenA) : address(tokenB);
        address token1 = address(tokenA) < address(tokenB) ? address(tokenB) : address(tokenA);
        (uint256 tokenId, uint128 liquidity, uint256 amount0, uint256 amount1) = nftManager.mint(INonfungiblePositionManager.MintParams({
            token0: token0,
            token1: token1,
            fee: FEE_MEDIUM,
            tickLower: -6000,
    ...
    function exchangeSwapExactInput(
        ...
        if (outputIsWETH) {
            data = new bytes[](2);
            data[0] = inputs;
            data[1] = abi.encodeWithSelector(router.unwrapWETH9.selector, amountOutMinimum, address(this));
        }
