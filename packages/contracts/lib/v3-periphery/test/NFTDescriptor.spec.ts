...
describe('NFTDescriptor', () => {
  ...
  const nftDescriptorFixture: Fixture<{
    ...
    const TestERC20Metadata = tokenFactory.deploy(
  ...
  describe('#tickToDecimalString', () => {
    ...
    describe('when tickspacing is 10', () => {
      ...
      it('returns MIN on lowest tick', async () => {
        expect(
          await nftDescriptor.tickToDecimalString(minTick, tickSpacing, 18, 18, false),
        ).to.equal('MIN');
      });

      it('returns MAX on the highest tick', async () => {
        expect(
          await nftDescriptor.tickToDecimalString(maxTick, tickSpacing, 18, 18, false),
        ).to.equal('MAX');
      });

      it('returns the correct decimal string when the tick is in range', async () => {
        expect(await nftDescriptor.tickToDecimalString(1, tickSpacing, 18, 18, false)).to.equal(
          '1.0001',
        );
      });
      ...
      it('returns the correct decimal string when tick is mintick for different tickspace', async () => {
        ...
        expect(
          await nftDescriptor.tickToDecimalString(otherMinTick, tickSpacing, 18, 18, false),
    ...
    describe('when tickspacing is 200', () => {
      ...
      it('returns the correct decimal string when the tick is in range', async () => {
        expect(await nftDescriptor.tickToDecimalString(0, tickSpacing, 18, 18, false)).to.equal(
          '1.0000',
        );
      });
    ...
    describe('when token ratio is flipped', () => {
      it('returns the inverse of default ratio for medium sized numbers', async () => {
        ...
        expect(await nftDescriptor.tickToDecimalString(10, tickSpacing, 18, 18, false)).to.eq(
          '1.0010',
        );
        expect(await nftDescriptor.tickToDecimalString(10, tickSpacing, 18, 18, true)).to.eq(
          '0.99900',
        );
      });
      ...
      it('returns the inverse of default ratio for small numbers', async () => {
        ...
        expect(await nftDescriptor.tickToDecimalString(-387272, tickSpacing, 18, 18, true)).to.eq(
          '65791000000000000',
        );
      });
      ...
      it('returns the correct string with differing token decimals', async () => {
        const tickSpacing = TICK_SPACINGS[FeeAmount.HIGH];
        expect(await nftDescriptor.tickToDecimalString(1000, tickSpacing, 18, 18, true)).to.eq(
          '0.90484',
        );
        expect(await nftDescriptor.tickToDecimalString(1000, tickSpacing, 18, 10, true)).to.eq(
          '90484000',
        );
        expect(await nftDescriptor.tickToDecimalString(1000, tickSpacing, 10, 18, true)).to.eq(
          '0.0000000090484',
        );
      });
      ...
      it('returns MIN for highest tick', async () => {
        ...
        expect(
          await nftDescriptor.tickToDecimalString(lowestTick, tickSpacing, 18, 18, true),
        ).to.eq('MAX');
      });
      ...
      it('returns MAX for lowest tick', async () => {
        ...
        expect(
          await nftDescriptor.tickToDecimalString(highestTick, tickSpacing, 18, 18, true),
        ).to.eq('MIN');
      });
    });
  });
    ...
    describe('when tokens have different decimal precision', () => {
      describe('when baseToken has more precision decimals than quoteToken', () => {
        it('returns the correct string when the decimal difference is even', async () => {
          expect(
            await nftDescriptor.fixedPointToDecimalString(encodePriceSqrt(1, 1), 18, 16),
          ).to.eq('100.00');
        });
        ...
        it('does not account for higher token0 precision if difference is more than 18', async () => {
          expect(await nftDescriptor.fixedPointToDecimalString(encodePriceSqrt(1, 1), 24, 5)).to.eq(
            '1.0000',
          );
        });
      });
      ...
      describe('when quoteToken has more precision decimals than baseToken', () => {
        it('returns the correct string when the decimal difference is even', async () => {
          expect(
            await nftDescriptor.fixedPointToDecimalString(encodePriceSqrt(1, 1), 10, 18),
          ).to.eq('0.000000010000');
        });
  ...
  describe('#tokenToColorHex', () => {
    ...
    it('returns the correct hash for the first 3 bytes of the token address', async () => {
      expect(await nftDescriptor.tokenToColorHex(tokens[0].address, 136)).to.eq(
        tokenToColorHex(tokens[0].address, 2),
      );
      expect(await nftDescriptor.tokenToColorHex(tokens[1].address, 136)).to.eq(
        tokenToColorHex(tokens[1].address, 2),
      );
    });

    it('returns the correct hash for the last 3 bytes of the address', async () => {
      expect(await nftDescriptor.tokenToColorHex(tokens[0].address, 0)).to.eq(
        tokenToColorHex(tokens[0].address, 36),
      );
      expect(await nftDescriptor.tokenToColorHex(tokens[1].address, 0)).to.eq(
