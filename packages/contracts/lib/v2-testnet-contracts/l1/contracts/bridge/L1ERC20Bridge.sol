...
contract L1ERC20Bridge is IL1Bridge, AllowListed, ReentrancyGuard {
  ...
  function finalizeWithdrawal(
    ...
    (address l1Receiver, address l1Token, uint256 amount) = _parseL2WithdrawalMessage(
      l2ToL1Message.data
    );
  ...
  function _verifyWithdrawalLimit(address _l1Token, uint256 _amount) internal {
    IAllowList.Withdrawal memory limitData = IAllowList(allowList).getTokenWithdrawalLimitData(
      _l1Token
    );
    ...
    if (block.timestamp > lastWithdrawalLimitReset[_l1Token] + 1 days) {
      ...
      require(
        _amount <= (limitData.withdrawalFactor * IERC20(_l1Token).balanceOf(address(this))) / 100,
        'w1'
      );
