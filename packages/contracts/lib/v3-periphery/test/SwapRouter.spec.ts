  ...
  describe('swaps', () => {
    ...
    describe('#exactInput', () => {
      async function exactInput(
        ...
        await expect(router.connect(trader).exactInput(params, { value })).to.be.revertedWith(
          'Too little received',
        );
        params.amountOutMinimum -= 1;
    ...
    describe('#exactOutput', () => {
      async function exactOutput(
        ...
        await expect(router.connect(trader).exactOutput(params, { value })).to.be.revertedWith(
          'Too much requested',
        );
        params.amountInMaximum += 1;
