...
contract Controller is Ownable, ReentrancyGuard, IERC721Receiver {
  ...
  function liquidate(
    ...
    uint256 bounty = _reduceDebt(
      cachedVault,
      IShortPerp(shortPerpNFT).ownerOf(_vaultId),
      _vaultId,
      true
    );
  ...
  function shutDown() external onlyOwner notShutdown {
    isSystemPaused = true;
    isShutDown = true;
    indexForSettlement = Power2Base._getScaledIndexTwap(indexOracle, TWAP_PERIOD);
  ...
  function _checkCanModifyVault(uint256 _vaultId, address _account) internal view {
    require(
      IShortPerp(shortPerpNFT).ownerOf(_vaultId) == _account ||
  ...
  function _depositUniPositionToken(
    ...
    ) = INonfungiblePositionManager(nftPositionManager).positions(_uniTokenId);
    ...
    require(
      (token0 == wLongPerpToken && token1 == stablecoin) ||
        (token1 == wLongPerpToken && token0 == stablecoin),
      'C23'
    );
  ...
  function _redeemUniToken(uint256 _uniTokenId) internal returns (uint256, uint256) {
    ...
    return
      isStablecoinToken0 ? (collectedToken0, collectedToken1) : (collectedToken1, collectedToken0);
  }
  ...
  function _getVaultStatus(
    ...
    return
      VaultLib.getVaultStatus(
        ...
        MIN_COLLATERAL,
        IExchangeOracle(marketOracle).getTimeWeightedAverageTickSafe(
          wLongPerpStablecoinPool,
          TWAP_PERIOD
        ),
        isStablecoinToken0
  ...
  function _getSingleLiquidationAmount(
    ...
    uint256 collateralToPay = Power2Base._getDebtValueInStablecoin(finalWAmountToLiquidate);
