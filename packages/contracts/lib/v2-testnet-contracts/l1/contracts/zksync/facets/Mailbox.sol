...
contract MailboxFacet is Base, IMailbox {
  ...
  function finalizeEthWithdrawal(
    ...
    bool proofValid = proveL2MessageInclusion(
      _l2BlockNumber,
      _l2MessageIndex,
      l2ToL1Message,
      _merkleProof
  ...
  function _verifyWithdrawalLimit(uint256 _amount) internal {
    IAllowList.Withdrawal memory limitData = IAllowList(s.allowList).getTokenWithdrawalLimitData(
      address(0)
    ); // address(0) denotes the ETH
    ...
    if (block.timestamp > s.lastWithdrawalLimitReset + 1 days) {
      ...
      require(
        _amount + s.withdrawnAmountInWindow <=
          (limitData.withdrawalFactor * address(this).balance) / 100,
        'w4'
      );
  ...
  function _verifyDepositLimit(address _depositor, uint256 _amount) internal {
    IAllowList.Deposit memory limitData = IAllowList(s.allowList).getTokenDepositLimitData(
      address(0)
    ); // address(0) denotes the ETH
  ...
  function _requestL2Transaction(
    ...
    {
      uint256 baseCost = _isFree
        ? 0
        : l2TransactionBaseCost(tx.gasprice, _l2GasLimit, _l2GasPerPubdataByteLimit);
  ...
  function _writePriorityOp(
    ...
    require(
      l2GasForTxBody / _priorityOpParams.l2GasPricePerPubdata <= PRIORITY_TX_MAX_PUBDATA,
      'uk'
    );
  ...
  function _getOverheadForTransaction(
    ...
    uint256 blockOverheadL2Gas = BLOCK_OVERHEAD_L2_GAS +
    ...
    uint256 overheadForLength = Math.ceilDiv(
      _encodingLength * blockOverheadL2Gas,
    ...
    {
      uint256 numerator = (blockOverheadL2Gas *
        _totalGasLimit +
        _gasPricePerPubdata *
  ...
  function _getTransactionBodyGasLimit(
    ...
    uint256 overhead = _getOverheadForTransaction(
      _totalGasLimit,
      _gasPricePerPubdata,
      _encodingLength
    );
