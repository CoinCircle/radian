...
describe('UniswapV3Pool', () => {
    ...
    describe('after initialization', () => {
      ...
      describe('failure cases', () => {
        ...
        it('fails if amount is 0', async () => {
          await expect(mint(wallet.address, minTick + tickSpacing, maxTick - tickSpacing, 0)).to.be
            .reverted;
        });
      });
      ...
      describe('success cases', () => {
        ...
        describe('above current price', () => {
          ...
          it('only clears the tick that is not used at all', async () => {
            ...
            let { liquidityGross, feeGrowthOutside0X128, feeGrowthOutside1X128 } = await pool.ticks(
              -240,
            );
            expect(liquidityGross).to.eq(0);
            expect(feeGrowthOutside0X128).to.eq(0);
  ...
  describe('#feeProtocol', () => {
    ...
    describe('#collectProtocol', () => {
      it('returns 0 if no fees', async () => {
        await pool.setFeeProtocol(6, 6);
        const { amount0, amount1 } = await pool.callStatic.collectProtocol(
          wallet.address,
          MaxUint128,
          MaxUint128,
        );
  ...
  it('tick transition cannot run twice if zero for one swap ends at fractional price just below tick', async () => {
    ...
    expect(await pool.liquidity(), 'current pool liquidity is still liquidity + 1').to.eq(
      liquidity,
    );
    ...
    expect(await pool.liquidity(), 'pool has run tick transition and liquidity changed').to.eq(
      liquidity.mul(2),
    );
  });
    ...
    describe('after liquidity added', () => {
      ...
      describe('fee off', () => {
        ...
        it('no-op if both amounts are 0', async () => {
          await expect(flash(0, 0, other.address))
            .to.not.emit(token0, 'Transfer')
            .to.not.emit(token1, 'Transfer');
        });
      ...
      describe('fee on', () => {
        ...
        it('increases the fee growth by the expected amount', async () => {
          ...
          const { token0: token0ProtocolFees, token1: token1ProtocolFees } =
  ...
  describe('#increaseObservationCardinalityNext', () => {
    ...
    describe('after initialization', () => {
      ...
      it('oracle starting state after initialization', async () => {
        const { observationCardinality, observationIndex, observationCardinalityNext } =
          await pool.slot0();
        expect(observationCardinality).to.eq(1);
        expect(observationIndex).to.eq(0);
  ...
  describe('#snapshotCumulativesInside', () => {
    ...
    it('throws if tick lower is too low', async () => {
      await expect(pool.snapshotCumulativesInside(getMinTick(tickSpacing) - 1, tickUpper)).be
        .reverted;
    });
    it('throws if tick upper is too high', async () => {
      await expect(pool.snapshotCumulativesInside(tickLower, getMaxTick(tickSpacing) + 1)).be
        .reverted;
    });
    it('throws if tick lower is not initialized', async () => {
      await expect(pool.snapshotCumulativesInside(tickLower - tickSpacing, tickUpper)).to.be
        .reverted;
    });
    it('throws if tick upper is not initialized', async () => {
      await expect(pool.snapshotCumulativesInside(tickLower, tickUpper + tickSpacing)).to.be
        .reverted;
    });
    ...
    it('relative behavior of snapshots', async () => {
      ...
      expect(tickCumulativeInside.sub(tickCumulativeInsideStart), 'tickCumulativeInside').to.eq(
        -20796,
      );
  ...
  describe('fees overflow scenarios', async () => {
    ...
    it('two positions at the same snapshot', async () => {
      ...
      let { amount0 } = await pool.callStatic.collect(
        wallet.address,
        minTick,
        maxTick,
        MaxUint128,
