  ...
  switch (askForUsage()) {
    case Usage.DEPLOY: {
      const addrsDeployed = {
        ...
        uniswapV3Factory: existingContractAddress('UniswapV3Factory'),
      ...
      if (!addrsDeployed.uniswapV3Factory) {
        const uniswapV3Factory = await trackDeployment(
          () => deploy.deployUniswapFactory(wallet!, gasOpts),
          `UniswapV3Factory`,
        );
        addrsDeployed.uniswapV3Factory = uniswapV3Factory.address;
      }
      if (!addrsDeployed.swapRouter) {
        const swapRouter = await trackDeployment(
          () => deploy.deployUniswapRouter(wallet!, addrsDeployed.uniswapV3Factory!, addrsDeployed.weth, gasOpts),
          `SwapRouter`,
        );
        ...
        const paymaster = await trackDeployment(
          () =>
            deploy.deployRadianPaymaster(
              wallet!,
              addrsDeployed.uniswapV3Factory!,
    ...
    case Usage.CALL: {
      ...
      const uniswapV3FactoryAddress = askForContract(`UniswapV3Factory`);
      const uniswapV3Factory: Types.UniswapV3Factory = await ethers.getContractAt(
        `UniswapV3Factory`,
...
+++++
    `);
    const wallet = zksync.Wallet.fromMnemonic(
      accounts.mnemonic,
      accounts.path + `/${accountNumber - 1}`,
    );
    return wallet.connect(new zksync.Provider(`https://zksync2-testnet.zksync.dev`));
  }
...
async function trackDeployment<T extends Contract>(
  ...
  for (;;) {
    try {
      ...
      console.log(`Failed to deploy ${name} contract, error: ${(e.message || ``).slice(0, 1000)}`);
