...
contract ExchangeOracle {
  ...
  function getHistoricalTwap(
    ...
    return
      _fetchHistoricTwap(_pool, _base, _quote, _secondsAgoToStartOfTwap, _secondsAgoToEndOfTwap);
  }
  ...
  function _fetchHistoricTwap(
    ...
    int24 twapTick = OracleLibrary.consultAtHistoricTime(
      _pool,
      _secondsAgoToStartOfTwap,
      _secondsAgoToEndOfTwap
    );
  ...
  function _getMaxPeriod(address _pool) internal view returns (uint32) {
    ...
    (uint32 oldestObservationTimestamp, , , bool initialized) = pool.observations(
