...
contract ExecutorFacet is Base, IExecutor {
  ...
  function _commitOneBlock(
    ...
    // Preventing "stack too deep error"
    {
      ...
      bool timestampNotTooBig = l2BlockTimestamp <=
      ...
      uint256 newStorageChangesIndexes = uint256(
        uint32(bytes4(_newBlock.initialStorageChanges[:4]))
      );
      require(
    ...
    )
  {
    ...
    for (uint256 i = 0; i < emittedL2Logs.length; i = i.uncheckedAdd(L2_TO_L1_LOG_SERIALIZE_SIZE)) {
      ...
      if (logSender == L2_TO_L1_MESSENGER) {
        ...
        require(
          bytecodeHash == L2ContractHelper.hashL2Bytecode(factoryDeps[currentBytecode]),
          'k3'
        );
  ...
  function commitBlocks(
    ...
    require(
      s.storedBlockHashes[s.totalBlocksCommitted] == _hashStoredBlockInfo(_lastCommittedBlockData),
      'i'
    ...
    for (uint256 i = 0; i < blocksLength; i = i.uncheckedInc()) {
      ...
      s.storedBlockHashes[_lastCommittedBlockData.blockNumber] = _hashStoredBlockInfo(
  ...
  function _collectOperationsFromPriorityQueue(
    uint256 _nPriorityOps
  ) internal returns (bytes32 concatHash) {
  ...
  function _executeOneBlock(
    ...
    bytes32 priorityOperationsHash = _collectOperationsFromPriorityQueue(
      _storedBlock.numberOfLayer1Txs
    );
  ...
  function proveBlocks(
    ...
    require(
      _hashStoredBlockInfo(_prevBlock) == s.storedBlockHashes[currentTotalBlocksVerified],
      't1'
    );
    ...
    for (uint256 i = 0; i < committedBlocksLength; i = i.uncheckedInc()) {
      ...
      require(
        _hashStoredBlockInfo(_committedBlocks[i]) ==
          s.storedBlockHashes[currentTotalBlocksVerified],
        'o1'
      );
    ...
    bool successVerifyProof = s.verifier.verify_serialized_proof(
      proofPublicInput,
  ...
  function _verifyRecursivePartOfProof(
  ...
  function _blockPassThroughData(
    CommitBlockInfo calldata _block
  ...
  function _blockAuxiliaryOutput(
    ...
    require(
      _block.initialStorageChanges.length <= MAX_INITIAL_STORAGE_CHANGES_COMMITMENT_BYTES,
      'pf'
    );
    require(
      _block.repeatedStorageChanges.length <= MAX_REPEATED_STORAGE_CHANGES_COMMITMENT_BYTES,
      'py'
    );
    ...
    return
      abi.encode(
        _block.l2LogsTreeRoot,
        l2ToL1LogsHash,
        initialStorageChangesHash,
        repeatedStorageChangesHash
      );
  }

  /// @notice Returns the keccak hash of the ABI-encoded StoredBlockInfo
  function _hashStoredBlockInfo(
