...
describe('NoDelegateCall', () => {
  ...
  const noDelegateCallFixture = async () => {
    ...
    const minimalProxyFactory = new ethers.ContractFactory(
      ...
      `3d602d80600a3d3981f3363d3d373d3d3d363d73${noDelegateCallTest.address.slice(
        2,
  ...
  it('runtime overhead', async () => {
    await snapshotGasCost(
      (
        await base.getGasCostOfCannotBeDelegateCalled()
      ).sub(await base.getGasCostOfCanBeDelegateCalled()),
    );
  });
