...
library VaultLib {
  ...
  // }
  function addStablecoinCollateral(Vault memory _vault, uint256 _amount) internal pure {
    _vault.collateralAmount = uint256(_vault.collateralAmount).add(_amount).toUint96();
  }
  ...
  // }
  function removeStablecoinCollateral(Vault memory _vault, uint256 _amount) internal pure {
    _vault.collateralAmount = uint256(_vault.collateralAmount).sub(_amount).toUint96();
  }
   ...
   * @param _wLongPerpPoolTick current price tick for wsqueeth pool
  ...
  function getVaultStatus(
    ...
    int24 _wLongPerpPoolTick,
    bool _isWethToken0
  ) internal view returns (bool, bool) {
    if (_vault.shortAmount == 0) return (true, false);

    uint256 debtValueInETH = uint256(_vault.shortAmount).mul(_normalizationFactor).div(ONE_ONE);
   ...
   * @param _positionManager address of the uniswap position manager
   * @param _normalizationFactor current _normalizationFactor
   * @param _wLongPerpPoolTick current price tick for wLongPerp / Stablecoin pool
   * @param _isStablecoinToken0 whether stablecoin is token0 in the wsqueeth pool
  ...
  function _getEffectiveCollateral(
    ...
    (uint256 nftStablecoinAmount, uint256 nftWLongPerpAmount) = _getUniPositionBalances(
      _positionManager,
    ...
    uint256 wSqueethIndexValueInStablecoin = nftWLongPerpAmount.mul(_normalizationFactor).div(
      ONE_ONE
    );
    // add eth value from uniswap position token as collateral
    return nftStablecoinAmount.add(wSqueethIndexValueInStablecoin).add(_vault.collateralAmount);
  }
   ...
   */
  function _getUniswapPositionInfo(
    address _positionManager,
    uint256 _tokenId
  ) internal view returns (int24, int24, uint128, uint128, uint128) {
