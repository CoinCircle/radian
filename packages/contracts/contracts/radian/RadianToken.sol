  ...
  ERC20FlashMint
{
  ...
  ) ERC20('Radian', 'RADIAN') ERC20Permit('Radian') {
  ...
  function _redeem(address _addr, address[] memory _assets, uint256 _radAmount) internal {
    ...
    for (uint256 i = 0; i < _assets.length; i++) {
      uint256 _collectorBalance = ERC20(_assets[i]).balanceOf(address(collector));
  ...
  // RAD <> RADX
  // =========================================================================
  function radianToRadianX(uint256 _amount) external {
    // Check balance
  ...
  function _vestedTransfer(
    ...
    require(
      ...
      'RadianToken::_vestedTransfer: insufficient balance for vested transfer'
    );
    ...
    // sanity check
    require(
      balanceOf(address(0)) >= prevBalanceOfNullAddress.add(_amount),
      'RadianToken:_vestedTransfer failed sanity check'
    );
  }
  ...
  function _releasableAmount(
    ...
    require(
      vestedTransfers[addr].length > i,
      'RadianToken::_releasableAmount: vesting index does not exist for address'
    );
  ...
  function _vestingProgress(address addr) internal view returns (uint256, uint256, uint256) {
    ...
    for (uint i = 0; i < vestedTransfers[addr].length; i++) {
      (uint256 releasedAmount, uint256 releasable, uint256 totalAmount) = _releasableAmount(
        addr,
        i
      );
