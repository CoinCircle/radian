...
contract('Governance', function(accounts) {
    ...
    describe("vote", () => {
        it("Should throw if a user chooses a number that is not valid", async () => {
            const rad = await RAD.new(CC_ADDRESS, COMMUNITY_ADDRESS);
            const governance = await Governance.new(START_TIMESTAMP, END_TIMESTAMP, NUM_OPTIONS, rad.address);
            for (let i = 0; i < 8; i++) {
        ...
        it("Should throw if the voting time has not yet begun", async () => {
            const rad = await RAD.new(CC_ADDRESS, COMMUNITY_ADDRESS);
            const governance = await Governance.new(START_TIMESTAMP + 5, END_TIMESTAMP, NUM_OPTIONS, rad.address);
            for (let i = 0; i < 8; i++) {
                await rad.transfer(accounts[i], ONE_MILLION.valueOf(), {from: CC_ADDRESS});
            }
            await rad.approve(governance.address, MAX_NUM, {from: USER_0});
        ...
        it("Should add to the total contribution of a user", async () => {
            ...
            assert.equal(ONE_MILLION.valueOf(), totalContribution.valueOf(), "Contribution did not get updated.");
        });
        it("Should add to the total contribution of a user after multiple transactions", async () => {
            ...
            assert.equal(ONE.plus(ONE_VOTE).valueOf(), totalContribution.valueOf(), "Contribution did not get updated.");

            await governance.vote(NUM_OPTIONS, ONE_VOTE, {from: USER_0});
            totalContribution = await governance.userContribution(USER_0);
            assert.equal(ONE.plus(ONE_VOTE).plus(ONE_VOTE).valueOf(), totalContribution.valueOf(), "Contribution did not get updated.");
        });
        it("Should add to the vote count of an option", async () => {
            ...
            assert.equal(ONE_MILLION.valueOf(), totalWeight.valueOf(), "Contribution did not get updated.");
        });
        it("Should add to the vote count of an option after multiple transactions", async () => {
            ...
            assert.equal(ONE.plus(ONE_VOTE).valueOf(), totalWeight.valueOf(), "Contribution did not get updated.");

            await governance.vote(NUM_OPTIONS, ONE_VOTE, {from: USER_0});
            totalWeight = await governance.optionVoteCount(NUM_OPTIONS);
            assert.equal(ONE.plus(ONE_VOTE).plus(ONE_VOTE).valueOf(), totalWeight.valueOf(), "Contribution did not get updated.");
        });
        it("Should transfer tokens from the user to the contract", async () => {
            ...
            assert.equal(ONE_MILLION.valueOf(), contractBalance.valueOf(), "Contribution did not get updated.");
        });
    ...
    describe("returnRAD", () => {
        ...
        it("Should return a user's RAD count to 0 after a contribution", async () => {
            ...
            let totalContribution = await governance.userContribution(USER_0);
            assert.equal(totalContribution.valueOf(), ONE_MILLION.valueOf(), "User did not receive his funds.")
    ...
    describe("fullTest", () => {
        it("Executes a full vote with 3 choices and 5 users voting. Choice 0 wins", async () => {
            ...
            assert.equal(ONE_MILLION.plus(HUNDRED_THOUSAND), voteWeight.valueOf(), "Vote count did not get updated.");

            voteWeight = await governance.optionVoteCount(ONE);
            assert.equal(TEN_THOUSAND.plus(ONE_THOUSAND), voteWeight.valueOf(), "Vote count did not get updated.");
            ...
            assert.equal(balanceOfUser.valueOf(), ONE_MILLION.valueOf(), "User did not get his RAD back.")
