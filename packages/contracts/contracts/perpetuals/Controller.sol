...
import {IWLongPerp} from "./interfaces/IWLongPerp.sol";
...
import {Uint256Casting} from "../lib/Uint256Casting.sol";
...
contract Controller is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    /// @dev address of the wLongPerp/usdx pool
    address public immutable wLongPerpStablecoinPool;
    address internal immutable nftPositionManager;
    address public immutable shortPerpNFT;
    address public immutable wLongPerpToken;
    /// @dev the cusom oracle for this perp
    address public immutable indexOracle;
    /// @dev the oracle for the exchange (to get mark price)
    ...
    bool internal immutable isStablecoinToken0;
     ...
     * @param _shortPerpNFT ERC721 token address representing the short position
     * @param _wLongPerpToken ERC20 token address representing the long position
     * @param _stablecoin quoteCurrency address
     * @param _wLongPerpStablecoinPool exchange pool for wPowerPerp / USDX
     * @param _nftPositionManager exchange position manager address
     * @param _feeTier Fee tier for liquidity pool
     * @param _weth WETH address
     */
    constructor(
        address _indexOracle,
        address _marketOracle,
        address _shortPerpNFT,
        address _wLongPerpToken,
        address _stablecoin,
        address _wLongPerpStablecoinPool,
        address _nftPositionManager,
        uint24 _feeTier,
        address _weth
    ) {
        require(_indexOracle != address(0), "C4");
        require(_marketOracle != address(0), "C4");
        require(_shortPerpNFT != address(0), "C5");
        require(_wLongPerpToken != address(0), "C6");
        require(_stablecoin != address(0), "C8");
        require(_wLongPerpStablecoinPool != address(0), "C9");
        require(_nftPositionManager != address(0), "C10");
        require(_feeTier > 0, "C11");

        indexOracle = _indexOracle;
        marketOracle = _marketOracle;
        shortPerpNFT = _shortPerpNFT;
        wLongPerpToken = _wLongPerpToken;
        stablecoin = _stablecoin;
        wLongPerpStablecoinPool = _wLongPerpStablecoinPool;
        nftPositionManager = _nftPositionManager;
        feeTier = _feeTier;
        weth = _weth;
        isStablecoinToken0 = stablecoin < wLongPerpToken;
     ...
     * @param _secondsAgo period which you want to calculate twap with
     * @return index price denominated in $USD, scaled by 1e18
     */
    function getIndex(uint32 _secondsAgo) external view returns (uint256) {
        return Power2Base._getIndex(_secondsAgo, indexOracle);
    }
    ...
    function getUnscaledIndex(uint32 _period) external view returns (uint256) {
        return Power2Base._getUnscaledIndex(_period, indexOracle);
    }

    /**
     * @notice get the expected mark price of powerPerp after funding has been applied
     * @param _twapSecondsAgo period of time for the twap in seconds
     * @return mark price denominated in $USD, scaled by 1e18
     */
    function getDenormalizedMark(uint32 _twapSecondsAgo) external view returns (uint256) {
        return
            Power2Base._getDenormalizedMark(
                _twapSecondsAgo,
                marketOracle,
                wLongPerpStablecoinPool,
                wLongPerpToken,
                stablecoin,
     ...
     * @param _twapSecondsAgo period which you want to calculate twap with
     * @return mark price denominated in $USD, scaled by 1e18
     */
    function getDenormalizedMarkForFunding(uint32 _twapSecondsAgo) external view returns (uint256) {
        return
            Power2Base._getDenormalizedMark(
                _twapSecondsAgo,
                marketOracle,
    ...
    function mintPowerPerpAmount(
        ...
        uint256 _uniTokenId
    ) external notPaused nonReentrant returns (uint256, uint256) {
        return _openDepositMint(msg.sender, _vaultId, _powerPerpAmount, _stablecoinAmount, _uniTokenId, false);
    }
    ...
    function mintWPowerPerpAmount(
        ...
        (uint256 vaultId, ) = _openDepositMint(msg.sender, _vaultId, _wPowerPerpAmount, _stablecoinAmount, _uniTokenId, true);
        return vaultId;
    }
     ...
     * @param _vaultId id of the vault
     */
    function deposit(uint256 _vaultId, uint256 _amount) external notPaused nonReentrant {
        _checkCanModifyVault(_vaultId, msg.sender);

        _applyFunding();
        VaultLib.Vault memory cachedVault = vaults[_vaultId];
        _addStablecoinCollateral(cachedVault, _vaultId, _amount);
    ...
    function reduceDebtShutdown(uint256 _vaultId) external isShutdown nonReentrant {
        VaultLib.Vault memory cachedVault = vaults[_vaultId];
        _reduceDebt(cachedVault, IShortPerp(shortPerpNFT).ownerOf(_vaultId), _vaultId, false);
        _writeVault(_vaultId, cachedVault);
    }
    ...
    function liquidate(uint256 _vaultId, uint256 _maxDebtAmount) external notPaused nonReentrant returns (uint256) {
        ...
        uint256 bounty = _reduceDebt(cachedVault, IShortPerp(shortPerpNFT).ownerOf(_vaultId), _vaultId, true);
        ...
        if (_isVaultSafe(cachedVault, cachedNormFactor)) {
            ...
            IERC20(stablecoin).transfer(msg.sender, bounty);
            return 0;
        }

        // add back the bounty amount, liquidators onlly get reward from liquidation
        cachedVault.addStablecoinCollateral(bounty);
        ...
        IERC20(stablecoin).transfer(msg.sender, collateralPaid);
    ...
    function updateOperator(uint256 _vaultId, address _operator) external {
        require(
            (shortPerpNFT == msg.sender) || (IShortPerp(shortPerpNFT).ownerOf(_vaultId) == msg.sender),
            "C20"
        );
    ...
    function shutDown() external onlyOwner notShutdown {
        isSystemPaused = true;
        isShutDown = true;
        indexForSettlement = Power2Base._getScaledIndexTwap(
            indexOracle,
            TWAP_PERIOD
        );
    ...
    function redeemLong(uint256 _wPerpAmount) external isShutdown nonReentrant {
        IWLongPerp(wLongPerpToken).burn(msg.sender, _wPerpAmount);
        ...
        IERC20(stablecoin).transfer(msg.sender, longValue);
    ...
    function redeemShort(uint256 _vaultId) external isShutdown nonReentrant {
        ...
        IERC20(stablecoin).transfer(msg.sender, excess);
    ...
    receive() external payable {
        // @TODO make this a utility func to accept eth
    ...
    function _checkCanModifyVault(uint256 _vaultId, address _account) internal view {
        require(
            IShortPerp(shortPerpNFT).ownerOf(_vaultId) == _account || vaults[_vaultId].operator == _account,
            "C20"
        );
    }
    ...
    function _openDepositMint(
        ...
        if (_depositAmount > 0) _addStablecoinCollateral(cachedVault, _vaultId, depositAmountWithFee);
        ...
        if (feeAmount > 0) IERC20(stablecoin).transfer(feeRecipient, feeAmount);
    ...
    function _burnAndWithdraw(
        ...
        if (_withdrawAmount > 0) IERC20(stablecoin).transfer(msg.sender, _withdrawAmount);
    ...
    function _openVault(address _recipient) internal returns (uint256, VaultLib.Vault memory) {
        uint256 vaultId = IShortPerp(shortPerpNFT).mintNFT(_recipient);
    ...
    function _depositUniPositionToken(
        ...
        require((token0 == wLongPerpToken && token1 == stablecoin) || (token1 == wLongPerpToken && token0 == stablecoin), "C23");

        _vault.addUniNftCollateral(_uniTokenId);
        INonfungiblePositionManager(nftPositionManager).safeTransferFrom(_account, address(this), _uniTokenId);
    ...
    // }
    function _addStablecoinCollateral(
        VaultLib.Vault memory _vault,
        uint256 _vaultId,
        uint256 _amount
    ) internal {
        IERC20(stablecoin).transferFrom(msg.sender, address(this), _amount);
        _vault.addStablecoinCollateral(_amount);
        emit DepositCollateral(msg.sender, _vaultId, _amount);
    }
    ...
    function _withdrawUniPositionToken(
        ...
        INonfungiblePositionManager(nftPositionManager).safeTransferFrom(address(this), _account, tokenId);
    ...
    function _withdrawCollateral(
        ...
        _vault.removeStablecoinCollateral(_amount);
        IERC20(stablecoin).transfer(msg.sender, _amount);
    ...
    function _mintWPowerPerp(
        ...
        IWLongPerp(wLongPerpToken).mint(_account, _wPowerPerpAmount);
    ...
    function _burnWPowerPerp(
        ...
        IWLongPerp(wLongPerpToken).burn(_account, _wPowerPerpAmount);
     ...
     * @dev this function will update the vault memory in-place
     * @param _vault the Vault memory to update
     * @param _maxWLongPerpAmount maximum debt amount liquidator is willing to repay
    ...
    function _liquidate(
        ...
        require(_maxWLongPerpAmount >= liquidateAmount, "C21");

        IWLongPerp(wLongPerpToken).burn(_liquidator, liquidateAmount);
        _vault.removeShort(liquidateAmount);
        _vault.removeStablecoinCollateral(collateralToPay);
    ...
    function _reduceDebt(
        ...
        (uint256 withdrawnStablecoinAmount, uint256 withdrawnWPowerPerpAmount) = _redeemUniToken(nftId);

        // change weth back to eth
        if (withdrawnStablecoinAmount > 0) IWETH9(weth).withdraw(withdrawnStablecoinAmount);
        ...
        if (excess > 0) IWLongPerp(wLongPerpToken).transfer(_owner, excess);
        if (burnAmount > 0) IWLongPerp(wLongPerpToken).burn(address(this), burnAmount);
     ...
     * @param _vault the Vault memory to update
     * @param _wLongPerpAmount the amount of wPowerPerpAmount minting
    ...
    function _getFee(
        ...
        if (_depositAmount > feeAmount) {
            ...
            _vault.removeStablecoinCollateral(feeAmount);
    ...
    function _redeemUniToken(uint256 _uniTokenId) internal returns (uint256, uint256) {
        INonfungiblePositionManager positionManager = INonfungiblePositionManager(nftPositionManager);

        (, , uint128 liquidity, , ) = VaultLib._getUniswapPositionInfo(nftPositionManager, _uniTokenId);
        ...
        return isStablecoinToken0 ? (collectedToken0, collectedToken1) : (collectedToken1, collectedToken0);
    }
    ...
    function _getNewNormalizationFactor() internal view returns (uint256) {
        ...
        uint256 index = Power2Base._getIndex(periodForOracle, indexOracle);
        ...
        returns (bool, bool)
    {
        return
            VaultLib.getVaultStatus(
                ...
                MIN_COLLATERAL,
                IExchangeOracle(marketOracle).getTimeWeightedAverageTickSafe(wLongPerpStablecoinPool, TWAP_PERIOD),
                isStablecoinToken0
            );
    }
    ...
    {
        uint256 bounty;
        if (_payBounty) bounty = _getReduceDebtBounty(nftStablecoinAmount, nftWPowerperpAmount);
        ...
        _vault.removeUniNftCollateral();
        _vault.addStablecoinCollateral(nftStablecoinAmount);
        _vault.removeStablecoinCollateral(bounty);
     ...
     * @param _stablecoinWithdrawn amount of eth withdrawn from uniswap by redeeming the position token
     * @param _wPowerPerpReduced amount of wPowerPerp withdrawn from uniswap by redeeming the position token
     */
    function _getReduceDebtBounty(uint256 _stablecoinWithdrawn, uint256 _wPowerPerpReduced) internal view returns (uint256) {
        return
            Power2Base
                ._getDebtValueInStablecoin(_wPowerPerpReduced)
                .add(_stablecoinWithdrawn)
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 collateralToPay = Power2Base._getDebtValueInStablecoin(
    ...
    function _getConsistentPeriodForOracle(uint32 _period) internal view returns (uint32) {
        uint32 maxPeriodPool1 = IExchangeOracle(marketOracle).getMaxPeriod(wLongPerpStablecoinPool);
        uint32 maxPeriodPool2 = IExchangeOracle(marketOracle).getMaxPeriod(wLongPerpStablecoinPool);
