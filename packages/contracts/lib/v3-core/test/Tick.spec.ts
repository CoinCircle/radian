...
describe('Tick', () => {
  ...
  describe('#tickSpacingToMaxLiquidityPerTick', () => {
    it('returns the correct value for low fee', async () => {
      const maxLiquidityPerTick = await tickTest.tickSpacingToMaxLiquidityPerTick(
  ...
  describe('#getFeeGrowthInside', () => {
    it('returns all for two uninitialized ticks if tick is inside', async () => {
      const { feeGrowthInside0X128, feeGrowthInside1X128 } = await tickTest.getFeeGrowthInside(
        -2,
        2,
        0,
        15,
  ...
  describe('#update', async () => {
    ...
    it('reverts on overflow liquidity gross', async () => {
      ...
      await expect(
        tickTest.update(0, 0, MaxUint128.div(2).sub(1), 0, 0, 0, 0, 0, false, MaxUint128),
      ).to.be.reverted;
    });
