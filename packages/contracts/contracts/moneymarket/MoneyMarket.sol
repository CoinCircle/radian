 ...
 * @author Compound
 */
contract MoneyMarketConfiguration {
...
abstract contract MoneyMarketCore is MoneyMarketConfiguration, MoneyMarketStorage, CometMath {
  /** Internal constants **/
 ...
 * @author Compound
 */
abstract contract IMoneyMarket is MoneyMarketCore {
  error Absurd();
 ...
 * Main contract
 */
contract MoneyMarket is ERC4626, IMoneyMarket {
  ...
  /// @notice Address of ERC20 stable coin
  // address public stablecoin;

  /// @notice Address of the Radian Exchange Factory
  address public exchange;

  /// @notice The curve used to calculate interest rates
  // ICurve public curve;

  mapping(address => AssetInfo) public markets;

  address[] public marketList;
  uint public numMarkets;

  /// @notice The minimum arithmetic mean tick for a market to be eligible to
  /// be added as a market
  int24 public minArithmeticMeanTick;

  /// @notice The minimum harmonic mean liquidity for a market to be eligible to
  /// be added as a market
  uint128 public minHarmonicMeanLiquidity;

  /// @notice Tracks the amount of collateral in each market that is owned
  /// by the protocol
  // mapping(address => uint256) public protocolOwnedCollateralAmounts;

  // ===========================================================================
  // COMPUTED STATE
  // ===========================================================================

  // /// @dev supply is always in stablecoin
  // function getSupplyRate(uint64 utilization) public view returns (uint) {
  //     uint y = curve.getPoint(utilization);
  //     return perSecondInterestRateBase[SUPPLY] + (1 / y);
  // }

  // function getBorrowRate(uint utilization) public view returns (uint) {
  //     uint y = curve.getPoint(utilization);
  //     return perSecondInterestRateBase[BORROW] + y;
  // }

  // function collateralValue(address asset, uint256 amount) public view returns (uint256) {
  //     // Ensure the market exists
  //     require(markets[asset].pool != address(0), "MARKET_NOT_FOUND");
  //     (uint160 sqrtPriceX96,,,,,,) = IExchangePool(markets[asset].pool).slot0();
  //     return sqrtPriceX96 * amount;
  // }

  // // borrow USD+ from the money market
  ...
  ) ERC4626(ERC20(config.baseToken), _name, _symbol) {
    // Sanity checks
    uint8 decimals_ = ERC20(config.baseToken).decimals();
    if (decimals_ > MAX_BASE_DECIMALS) revert BadDecimals();
    if (config.storeFrontPriceFactor > FACTOR_SCALE) revert BadDiscount();
    if (config.baseMinForRewards == 0) revert BadMinimum();
    if (IPriceFeed(config.baseTokenPriceFeed).decimals() != PRICE_FEED_DECIMALS)
      revert BadDecimals();
   ...
   */
  function getAssetInfoByAddress(address asset) public view override returns (AssetInfo memory) {
   ...
   * @param asset The address of a price feed
  ...
  function getPrice(address asset) public view override returns (uint) {
    ...
    address pool = address(markets[asset].pool);
    (int24 arithmeticMeanTick, ) = OracleLibrary.consult(pool, 60 * 60 * 24 * 30);
    uint256 quote = OracleLibrary.getQuoteAtTick(arithmeticMeanTick, 1e18, baseToken, asset);
    return quote;
  }
  ...
  function isBorrowCollateralized(address account) public view override returns (bool) {
    ...
    address[] memory assetsIn = userBasic[account].assetsIn;
    ...
    for (uint16 i = 0; i < numMarkets; ) {
      address assetAddress = assetsIn[i];
      if (userCollateral[account][assetsIn[i]].balance > 0) {
        if (liquidity >= 0) {
          return true;
        }

        AssetInfo memory asset = getAssetInfoByAddress(assetAddress);
  ...
  function isLiquidatable(address account) public view override returns (bool) {
    ...
    for (uint16 i = 0; i < assetsIn.length; ) {
      address assetAddress = assetsIn[i];
      if (userCollateral[account][assetsIn[i]].balance > 0) {
        if (liquidity >= 0) {
          return false;
  ...
  function updateAssetsIn(
    ...
    if (initialUserBalance == 0 && finalUserBalance != 0) {
      // enter
      userBasic[account].assetsIn[userBasic[account].assetsIn.length] = assetInfo.asset;
    } else if (initialUserBalance != 0 && finalUserBalance == 0) {
      // exit
      if (userBasic[account].assetsIn.length > 1) {
        for (uint i = 0; i < userBasic[account].assetsIn.length; i++) {
          if (userBasic[account].assetsIn[i] == assetInfo.asset) {
            userBasic[account].assetsIn[i] = userBasic[account].assetsIn[
              userBasic[account].assetsIn.length - 1
            ];
            break;
          }
        }
      }
      userBasic[account].assetsIn.pop();
    }
  }
  ...
  function transferFrom(
    ...
    address dst,
    uint amount
  ) public override(ERC20, IMoneyMarket) returns (bool) {
  ...
  function absorbInternal(address absorber, address account) internal {
    ...
    address[] memory assetsIn = accountUser.assetsIn;

    uint256 basePrice = getPrice(baseTokenPriceFeed);
    uint256 deltaValue = 0;
    // to avoid stack too deep error
    address absorber_ = absorber;

    for (uint8 i = 0; i < accountUser.assetsIn.length; ) {
      if (userCollateral[account][assetsIn[i]].balance > 0) {
        AssetInfo memory assetInfo = getAssetInfoByAddress(accountUser.assetsIn[i]);
        address asset = assetInfo.asset;
        uint128 seizeAmount = userCollateral[account][asset].balance;
        userCollateral[account][asset].balance = 0;
        totalsCollateral[asset].totalSupplyAsset -= seizeAmount;

        uint256 value = mulPrice(seizeAmount, getPrice(assetInfo.asset), assetInfo.scale);
    ...
    userBasic[account].assetsIn = empty;
    ...
    emit AbsorbDebt(absorber_, account, basePaidOut, valueOfBasePaidOut);
   ...
   */
  function quoteCollateral(address asset, uint baseAmount) public view override returns (uint) {
    AssetInfo memory assetInfo = getAssetInfoByAddress(asset);
    uint256 assetPrice = getPrice(assetInfo.asset);
   ...
   */
  function approveThis(address manager, address asset, uint amount) external override {
    if (msg.sender != governor) revert Unauthorized();
   ...
   *
   * * Compound computes this value to account for accrued interest, but there is
   * already a ERC4626 standard way of getting this. Most wallets would show
   * an incorrect balance if this was used.
   **/
  function totalSupplyNow() external view returns (uint256) {
   ...
   * an incorrect balance if this was used.
   */
  function balanceOfNow(address account) public view returns (uint256) {
    (uint64 baseSupplyIndex_, ) = accruedInterestIndices(getNowInternal() - lastAccrualTime);
    int104 principal = userBasic[account].principal;
  ...
  ////////////////////////////////////////////////////////////////////////////
  ///////////////////////////////////////////////////////////////////////////
  // Radian Exchange integration
  //

  function addMarket(address asset, address pool, uint8 poolFee, uint128 supplyCap) public {
    require(poolExists(pool, poolFee), 'Pool does not exist');
    require(isValidPool(asset, pool), 'Pool does not belong to asset');
    require(isPoolHealthy(asset, pool), 'Pool is not healthy');
    require(!marketExists(asset), 'Market is already added');
    uint64 scale = ERC20(asset).decimals();
    markets[asset].pool = pool;
    markets[asset].asset = asset;
    markets[asset].scale = scale;
    markets[asset].borrowCollateralFactor = 0.65e18; // 65%
    markets[asset].liquidateCollateralFactor = 0.85e18; // 85%
    markets[asset].liquidationFactor = 7e16; // 7%
    markets[asset].supplyCap = supplyCap;
    // emit MarketAdded(asset, pool, poolFee, supplyCap);
  }

  function isValidPool(address asset, address pool) public view returns (bool) {
    return IExchangePool(pool).token0() == asset || IExchangePool(pool).token1() == asset;
  }

  function isPoolHealthy(address asset, address pool) public view returns (bool) {
    (, uint128 harmonicMeanLiquidity) = OracleLibrary.consult(pool, 60 * 60 * 24 * 30);
    return harmonicMeanLiquidity > minHarmonicMeanLiquidity;
  }

  function poolExists(address asset, uint24 fee) public view returns (bool) {
    return IExchangePoolFactory(exchange).getPool(asset, baseToken, fee) != address(0);
  }

  function marketExists(address asset) public view returns (bool) {
    return markets[asset].pool != address(0);
  }
  ...
  function maxWithdraw(address owner) public view override returns (uint256) {
    return convertToAssets(balanceOfNow(owner));
  }

  function maxRedeem(address owner) public view override returns (uint256) {
    return balanceOfNow(owner);
  }
