...
contract RadianPaymaster is IPaymaster {
  ...
  event PaymasterTransactionExecuted(
  ...
  constructor(address _uniswapV3FactoryAddress, address _swapRouterAddress, address _wethAddress) {
  ...
  function getPoolAddress(address _erc20) public view returns (address, uint24) {
    // Sort
    (address token0, address token1) = _erc20 < WETH_ADDRESS
      ? (_erc20, WETH_ADDRESS)
      : (WETH_ADDRESS, _erc20);
  ...
  function validateAndPayForPaymasterTransaction(
    ...
    if (paymasterInputSelector == IPaymasterFlow.approvalBased.selector) {
      (address token, uint256 minAllowance, bytes memory data) = abi.decode(
      ...
      TransferHelper.safeApprove(token, SWAP_ROUTER_ADDRESS, providedAllowance);
      ISwapRouter.ExactOutputSingleParams memory params = ISwapRouter.ExactOutputSingleParams({
        ...
        amountInMaximum: providedAllowance,
        sqrtPriceLimitX96: 0
      });
      ISwapRouter swapRouter = ISwapRouter(SWAP_ROUTER_ADDRESS);
      // Executes the swap returning the amountIn needed to spend to receive the desired amountOut.
      // This fails if the swap doesn't result in the right amount of ETH
      uint256 amountSpent = swapRouter.exactOutputSingle(params);
      emit PaymasterTransactionExecuted(
        token,
        providedAllowance,
        requiredETH,
        providedAllowance - amountSpent
      );
