        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          ...
          async function simulateSwap(
            ...
            const { amount0Delta, amount1Delta, nextSqrtRatio } =
              await tester.callStatic.getSwapResult(
                pool.address,
                zeroForOne,
                amountSpecified,
          ...
          for (const { zeroForOne, assumedTruePriceAfterSwap, inputAmount, description } of [
            ...
            describe(description, () => {
              ...
              it('sandwiched with swap to execution price then mint max liquidity/target/burn max liquidity', async () => {
                ...
                const priceSwapStart = await tickMath.getSqrtRatioAtTick(
                ...
                // sub the mint costs
                const { amount0: amount0Mint, amount1: amount1Mint } =
                  pool.interface.decodeEventLog(
                    pool.interface.events[
