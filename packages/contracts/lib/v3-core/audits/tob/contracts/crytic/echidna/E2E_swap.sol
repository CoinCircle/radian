...
contract E2E_swap {
  ...
  function check_liquidityNet_invariant() internal {
    ...
    for (uint256 i = 0; i < usedTicks.length; i++) {
      ...
      assert(
        (tickLiquidityNet >= 0 && result >= liquidityNet) ||
          (tickLiquidityNet < 0 && result < liquidityNet)
      );
      liquidityNet = result;
    }
  ...
  function check_liquidity_invariant() internal {
    ...
    for (uint256 i = 0; i < usedTicks.length; i++) {
      ...
      if (tick <= currentTick) {
        ...
        assert(
          (tickLiquidityNet >= 0 && result >= liquidity) ||
            (tickLiquidityNet < 0 && result < liquidity)
        );
        liquidity = result;
      }
  ...
  function check_tick_feegrowth_invariant() internal {
    ...
    (, , uint256 tB_feeGrowthOutside0X128, uint256 tB_feeGrowthOutside1X128) = pool.ticks(
      tickBelow
    );
    (, , uint256 tA_feeGrowthOutside0X128, uint256 tA_feeGrowthOutside1X128) = pool.ticks(
      tickAbove
    );
  ...
  function viewRandomInit(
    ...
    poolPositions = forgePoolPositions(
      _seed,
  ...
  function forgePoolParams(uint128 _seed) internal view returns (PoolParams memory poolParams) {
    ...
    //
    poolParams.startTick = int24(
      (_seed % uint128(poolParams.tickCount)) * uint128(poolParams.tickSpacing)
    );
    if (_seed % 3 == 0) {
      // set below 0
  ...
  function forgePoolPositions(
    ...
    for (uint8 i = 0; i < positionsCount; i++) {
      ...
      if (_seed % 2 == 0) {
        ...
        uint24 randomIncrement = uint24(
          (_seed % uint128(poolTickCountTotalLeft)) * uint128(_poolTickSpacing)
        );
  ...
  function test_swap_exactIn_zeroForOne(uint128 _amount) public {
    ...
    (UniswapSwapper.SwapperStats memory bfre, UniswapSwapper.SwapperStats memory aftr) = swapper
      .doSwap(true, _amountSpecified, sqrtPriceLimitX96);
  ...
  function test_swap_exactIn_oneForZero(uint128 _amount) public {
    ...
    (UniswapSwapper.SwapperStats memory bfre, UniswapSwapper.SwapperStats memory aftr) = swapper
      .doSwap(false, _amountSpecified, sqrtPriceLimitX96);
