...
describe('SqrtPriceMath', () => {
  ...
  describe('#getNextSqrtPriceFromInput', () => {
    ...
    it('fails if input amount overflows the price', async () => {
      ...
      const amountIn = 1024;
      await expect(sqrtPriceMath.getNextSqrtPriceFromInput(price, liquidity, amountIn, false)).to.be
        .reverted;
    });
    ...
    it('any input amount cannot underflow the price', async () => {
      ...
      expect(await sqrtPriceMath.getNextSqrtPriceFromInput(price, liquidity, amountIn, true)).to.eq(
        1,
      );
    });
    ...
    it('returns the minimum price for max inputs', async () => {
      ...
      expect(
        await sqrtPriceMath.getNextSqrtPriceFromInput(sqrtP, liquidity, maxAmountNoOverflow, true),
      ).to.eq('1');
    });
  ...
  describe('#getNextSqrtPriceFromOutput', () => {
    ...
    it('fails if output amount is exactly the virtual reserves of token0', async () => {
      ...
      const liquidity = 1024;
      const amountOut = 4;
      await expect(sqrtPriceMath.getNextSqrtPriceFromOutput(price, liquidity, amountOut, false)).to
        .be.reverted;
    });
    ...
    it('fails if output amount is greater than virtual reserves of token1', async () => {
      ...
      await expect(sqrtPriceMath.getNextSqrtPriceFromOutput(price, liquidity, amountOut, true)).to
        .be.reverted;
    });
